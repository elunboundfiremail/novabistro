...
async function cargarAdmin() {
    ...
    lista_personal_local.forEach(p => { lP.innerHTML += `<tr class="hover:bg-neutral-800/50 text-xs border-b border-neutral-800 text-center"><td class="p-4 font-black text-amber-600">${p.ci}</td><td class="p-4 text-white uppercase font-bold text-left">${p.nombre}</td><td class="p-4 uppercase text-left">${p.apellido_paterno}</td><td class="p-4 uppercase text-left">${p.apellido_materno || '-'}</td><td class="p-4 font-mono">${p.telefono || '-'}</td><td class="p-4 text-[8px] font-black uppercase">${roles_nom[p.id_rol]}</td><td class="p-4 text-center flex gap-2 justify-center"><button class="bg-blue-900/50 text-blue-400 border border-blue-800 text-[9px] px-3 py-1 rounded font-black" onclick="abrirFormPersonal(${p.id_personal})">Editar</button><button class="bg-red-900/50 text-red-400 border border-red-800 text-[9px] px-3 py-1 rounded font-black" onclick="borrar('personal', ${p.id_personal})">Baja</button></td></tr>`; });
    ...
    for(let cat in ag) {
        ...
        ag[cat].forEach(x => { lPr.innerHTML += `<tr class="border-b border-neutral-800 text-xs hover:bg-neutral-800/50"><td class="p-3 uppercase font-bold text-neutral-300 text-left">${x.nombre}</td><td class="p-3 text-amber-500 font-black">Bs. ${parseFloat(x.precio_bs).toFixed(2)}</td><td class="p-3 text-center flex gap-2 justify-center"><button class="bg-blue-900 text-white text-[9px] px-2 py-1 rounded font-black uppercase" onclick="abrirFormProducto(${x.id_producto})">Edit</button><button class="bg-red-900 text-white text-[9px] px-2 py-1 rounded font-black uppercase" onclick="borrar('productos', ${x.id_producto})">Baja</button></td></tr>`; });
    }
}
